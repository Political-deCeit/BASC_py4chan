...
from .url import Url
from .file import File4Chan, FileFoolFuuka
from .util import clean_comment_body, is_4chan

class Post4Chan(object):
    ...
    def __init__(self, thread, data):
        self._thread = thread
        self._domain = thread._domain
        self._data = data
        self._url = Url(board_name=self._thread._board.name, domain=self._domain)		# 4chan URL generator

        # add file objects if they exist
        if self.has_file:
            self.file1 = File4Chan(self, self._data)
...
class PostFoolFukka(Post4Chan):
    """Represents a FoolFukka (archive) post."""

    def __init__(self, thread, data):
        super(PostFoolFukka, self).__init__(thread, data)
        # override the file object with the FoolFuuka one
        if self.has_file:
            self.file1 = FileFoolFuuka(self, self._data["media"])
        a = 1
