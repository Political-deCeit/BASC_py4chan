#!/usr/bin/env python
# -*- coding: utf-8 -*-
from .post import Post4Chan, PostFoolFukka
from .url import Url
...
class Thread(object):
    ...
    def __init__(self, board, id):
        self._board = board
        self._domain = board._domain
        self._url = Url(board_name=board.name)       # 4chan URL generator
    ...
    @classmethod
    def _from_4chan_json(cls, json, board, id=None, last_modified=None):
        t = cls(board, id)
        ...
        t.topic = t.op = Post4Chan(t, head)
        t.replies.extend(Post4Chan(t, p) for p in rest)
    ...
    @classmethod
    def _from_foolfuuka_json(cls, json:dict, board, id=None, last_modified=None):
        t = cls(board, id)
        t._last_modified = last_modified

        json_root = next(iter(json.values()))

        posts = json_root['posts']
        head, rest = json_root['op'], posts

        t.topic = t.op = PostFoolFukka(t, head)
        t.replies.extend(PostFoolFukka(t, p) for p in rest.values())
    ...
    def _from_json(cls, json, board, id=None, last_modified=None):
        if is_4chan(board._domain):
            return cls._from_4chan_json(json, board, id, last_modified)
        else:
            return cls._from_foolfuuka_json(json, board, id, last_modified)
    ...
    def update(self, force=False):
        ...
        # 404 Not Found, thread died.
            ...
            self.topic = Post4Chan(self, posts[0])

            if self.last_reply_id and not force:
                self.replies.extend(Post4Chan(self, p) for p in posts if p['no'] > self.last_reply_id)
            else:
                self.replies[:] = [Post4Chan(self, p) for p in posts[1:]]
